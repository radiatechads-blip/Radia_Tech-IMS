...
export function PendingMaterialBillsPageContent() {
  ...
  // ---- Bill Details ----
  const [billingDate, setBillingDate] = useState(today);
  const [updateNumber, setUpdateNumber] = useState(() => `PMB-${Date.now().toString().slice(-6)}`);
  const [preparedBy, setPreparedBy] = useState(companyInfo?.ceo || "Authorized Signatory");
  const [subject, setSubject] = useState("");
  const [taxType, setTaxType] = useState<TaxType>("cgst-sgst");
  const [globalTaxPercent, setGlobalTaxPercent] = useState(18);
  ...
  useEffect(() => {
    ...
    const loadInvoice = async () => {
      try {
        ...
        setCustomerCompany(mapped.customerCompany);
        setCustomerPhone(mapped.customerPhone);
        setCustomerEmail(mapped.customerEmail);
        setCustomerGst(mapped.customerGst);
        setCustomerAddress(mapped.customerAddress);
        setCustomerCity(mapped.customerCity);
        setCustomerState(mapped.customerState);
        setCustomerPincode(mapped.customerPincode);
        setBillingDate(mapped.billingDate);
        setUpdateNumber(mapped.invoiceNumber || `PMB-${Date.now().toString().slice(-6)}`);
        setPreparedBy(mapped.preparedBy);
        setSubject("");
        setNotes(mapped.notes);
        setTerms(mapped.terms);
        setRoundOffAmount(mapped.roundOffAmount);
        setTaxType(mapped.taxType);
        setGlobalTaxPercent(mapped.globalTaxPercent);
  ...
  // Construct DNPreview Invoice Data Object
  const previewData: InvoiceSummary = useMemo(() => {
    return {
      billType: "Pending Material Bill",
      partyName: customerName || customerCompany || "—",
      contactPerson: customerName && customerCompany ? customerName : "",
      ...
      notes,
      terms,
      authorizedSignature: preparedBy,
      taxType,
      items: items.map((item) => ({
        ...
        unit: item.unit,
        rate: item.rate,
        taxPercent: taxType !== "none" ? globalTaxPercent : 0,
      })),
    };
  }, [
    customerName,
...
<html>
  <head>
    <meta charset="utf-8" />
    <base href="${origin}" />
    <title>${previewData.billType || "Pending Material Bill"} Preview</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
      @page { size: A4; margin: 10mm; }
      @media print {
        body { margin: 0; background: #fff; }
        .invoice-preview-shell { width: 100%; }
      }
    </style>
  </head>
  <body style="margin:0;padding:24px;background:#f8fafc;font-family:Arial,Helvetica,sans-serif;"></body>
</html>`);
    ...
    root.render(<PMPreview invoice={previewData} taxType={taxType} />);
  ...
  const handleSaveInvoice = async () => {
    if (!customerName.trim() && !customerCompany.trim()) {
      window.alert("Please enter a customer name or company before saving.");
      return;
    }
    ...
    try {
      const payload = {
        ...
        partyName: customerName || customerCompany || "Customer",
        contactPerson: customerName && customerCompany ? customerName : customerName || customerCompany || "",
        address: customerAddress,
        city: customerCity,
        state: customerState,
        pincode: customerPincode,
        ...
        items: items
          ...
          .map((item) => {
            ...
            const gstAmount = taxType === "none" ? 0 : (taxableAmount * globalTaxPercent) / 100;
            return {
              description: item.productName,
              hsn: "",
              unit: item.unit,
              qty: item.qty,
              rate: item.rate,
              taxPercent: taxType === "none" ? 0 : globalTaxPercent,
              discountPercent: 0,
              taxablePerUnit: item.rate,
              taxableAmount,
              gstAmount,
              finalRatePerUnit: item.rate,
              rowAmount: taxableAmount + gstAmount,
            };
          }),
      };
      ...
      if (!response.ok) {
        throw new Error(data?.error || "Unable to save pending material bill.");
      }

      window.alert(isEditing ? "Pending Material bill updated successfully." : "Pending Material bill saved and added to Generate Bill history.");
      router.push("/admin/generate-bill");
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unable to save pending material bill.";
    ...
    >
      ...
      <div className="min-h-screen bg-[#e8eaf0] font-sans text-[13px]">
        ...
        <div className={`${showPreview ? "hidden" : ""} print:hidden`}>
          ...
          <div className="mx-auto max-w-350 p-3 space-y-2">
            {/* Customer Lookup and Metadata Details Grid */}
            <div className="rounded bg-white border border-gray-200 shadow-sm p-3">
              <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
                {/* Customer Details Column */}
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-[12px]" style={{ minWidth: "800px" }}>
                  <thead>
                    <tr className="bg-[#e8eaf0] text-left text-slate-700">
                      <th className="border border-slate-300 px-3 py-2 font-semibold w-12">#</th>
                      <th className="border border-slate-300 px-3 py-2 font-semibold min-w-62.5">Product / Line Description</th>
                      <th className="border border-slate-300 px-3 py-2 font-semibold w-24 text-right">Qty</th>
                      <th className="border border-slate-300 px-3 py-2 font-semibold w-28">Unit</th>
                      <th className="border border-slate-300 px-3 py-2 font-semibold w-32 text-right">MRP / Rate (Rs)</th>
                      <th className="border border-slate-300 px-3 py-2 font-semibold w-36 text-right">Calculated Total</th>
        ...
        {/* HIGH-FIDELITY PRINTABLE PREVIEW USING DNPreview */}
        <div className={showPreview ? "block bg-slate-100 min-h-screen p-4 sm:p-6" : "hidden print:block print:bg-white print:p-0 print:min-h-0"}>
          <div className="mx-auto mb-4 flex max-w-5xl items-center justify-between border-b border-slate-300 pb-3 print:hidden">
            <span className="text-sm font-bold text-slate-800">Document Layout Preview View</span>
            <button
              type="button"
              onClick={() => setShowPreview(false)}
              className="rounded border border-slate-300 bg-white px-3.5 py-1.5 text-xs font-semibold text-slate-700 shadow-sm hover:bg-slate-50"
            >
...
export default function PendingMaterialBillsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50 p-4 text-sm text-slate-600">Loading pending material form…</div>}>
      <PendingMaterialBillsPageContent />
    </Suspense>
  );
}