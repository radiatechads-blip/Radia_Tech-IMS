...
export default function AdminCategoriesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this category? Products in it will also be deleted."))
      return;
    const res = await fetch(`/api/categories/${id}`, { method: "DELETE" });
    if (res.ok)
  ...
  return (
    <AdminShell
      ...
      action={
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-dark hover:shadow sm:w-auto cursor-pointer"
        >
          <Plus size={16} /> Add Category
        </button>
      }
    >
          ...
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Keep names short and descriptions useful for catalogue browsing.
              </p>
            </div>
            <button
              onClick={resetForm}
              className="rounded-full p-1 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 cursor-pointer"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
              />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-700">
              ...
              <textarea
                ...
                className="w-full resize-none rounded-lg border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
              />
            </div>
            <div className="flex flex-col gap-2 pt-2 sm:flex-row">
              <button
                type="submit"
                className="rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-dark hover:shadow cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={resetForm}
                className="rounded-lg border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 cursor-pointer"
              >
                Cancel
              </button>
            </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-red-200 bg-red-50 px-5 py-6 text-sm font-medium text-red-700">
          {error}
        </div>
      ) : categories.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {categories.map((category) => (
            ...
            >
              ...
              <p className="line-clamp-3 min-h-15 text-sm text-slate-600">
              ...
              <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-sm font-semibold text-primary transition hover:bg-primary/10 hover:text-primary-dark cursor-pointer"
                  >
                    <Edit3 size={15} /> Edit
                  </button>
                  <button
                    onClick={() => handleDelete(category.id)}
                    className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-sm font-semibold text-red-600 transition hover:bg-red-50 hover:text-red-700 cursor-pointer"
                  >
