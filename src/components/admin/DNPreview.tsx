...
export default function DNPreview({ invoice, taxType: taxTypeProp, pageLabels, onTaxTypeChange }: Props) {
        ...
        {labels.map((label, index) => (
          <section key={`${label}-${index}`} className={`invoice-preview-page ${index > 0 ? "mt-6 border-t border-dashed border-slate-300 pt-6 print:mt-0 print:border-t-0 print:pt-0" : ""}`} style={index > 0 ? { breakBefore: "page", pageBreakBefore: "always" } : undefined}>
            <div className="relative flex items-center justify-center border-b border-slate-300 bg-[#f7f9fc] px-6 py-3">
              <h2 className="text-base font-semibold text-slate-900">Debit Note</h2>
              <span className="absolute right-6 text-[11px] font-semibold uppercase tracking-wide text-slate-500">{label}</span>
            </div>
            <div className="border-b border-slate-300 bg-white px-5 py-3">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-[18px] font-extrabold tracking-wide text-slate-950">RADIATECH ELECTRA</h3>
                    <p className="mt-0.5 text-[12px] leading-5 text-slate-500">
                      Basement, A-287, Sector 69, Noida, Gautam Buddha Nagar, Uttar Pradesh, 201301
                    </p>
                  </div>
                </div>
                <div className="text-right text-[12px] leading-5 text-slate-600">
            ...
            <div className="grid border-b border-slate-300 md:grid-cols-2">
              <div className="border-b border-slate-300 md:border-b-0 md:border-r">
                <div className={secHeader}>Return To:</div>
              ...
              <div>
                ...
                <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 p-3 text-[13px] text-slate-800">
                  <span className="font-semibold text-slate-900">Return No:</span><span>{getDuplicateCopyInvoiceNumber(invoice.invoiceNumber, false) || "—"}</span>
                  <span className="font-semibold text-slate-900">Date:</span><span>{fmtDate(invoice.invoiceDate || invoice.createdAt)}</span>
                  <span className="font-semibold text-slate-900">Bill Date:</span><span>{fmtDate(invoice.poDate)}</span>
                  <span className="font-semibold text-slate-900">E-way Bill No:</span><span>{invoice.ewayBillNo || "—"}</span>
                  <span className="font-semibold text-slate-900">Bill No:</span><span>{invoice.poNo || "—"}</span>
                  <span className="font-semibold text-slate-900">Place of Supply:</span><span>{invoice.placeOfSupply || "—"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid border-b border-slate-300 md:grid-cols-2">
              <div className="border-b border-slate-300 md:border-b-0 md:border-r">
                <div className={secHeader}>Ship To:</div>
                <div className="p-3 whitespace-pre-line text-[12px] leading-5 text-slate-800">{invoice.shipToAddress || "—"}</div>
              </div>
              <div>
                ...
                <tbody>
                  {rows.map((item, i) => (
                    <tr key={item.id || i} className="odd:bg-white even:bg-slate-50/50">
                      ...
                      <td className="border border-slate-300 px-1.5 py-1.5 text-right align-top font-semibold text-slate-900">{fmt(item.rowAmount)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
            ...
            <div className="grid border-b border-slate-300 md:grid-cols-[1fr_280px]">
              <div className="border-b border-slate-300 md:border-b-0 md:border-r">
                ...
                <div className="p-3">
                  <div className="mb-2 print:hidden">
                    <span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">Tax Option</span>
                    <select
                      value={taxType}
                  ...
                  <table className="w-full border-collapse text-[11px]">
                    ...
                    <tbody>
                      <tr>
                        <td className="border border-slate-300 px-2 py-1 font-semibold text-slate-900">{fmt(totals.taxable)}</td>
                        {taxType === "cgst-sgst" ? (
                          <>
              ...
              <div className="p-3 text-[12px] text-slate-800">
                ...
                <div className="mt-2 flex justify-between gap-2 border-t border-slate-300 pt-2 text-[14px] font-bold text-slate-950">
                ...
                <div className="mt-0.5 flex justify-between gap-2 font-semibold text-slate-900">
                  <span>Balance</span>
                  <span className="shrink-0 text-right">: {fmt(totals.grandTotal)}</span>
                </div>
              </div>
            ...
            {(hasNotes || hasTerms) && (
              <div className="grid border-b border-slate-300 md:grid-cols-2">
                {hasNotes && (
                  <div className={`border-b border-slate-300 md:border-b-0 md:border-r ${hasTerms ? "" : ""}`}>
                    <div className={secHeader}>Notes</div>
                    <div className="whitespace-pre-line px-4 py-2.5 text-[12px] leading-5 text-slate-700">{invoice.notes}</div>
                  </div>
                )}
                {hasTerms && (
                  <div>
                    <div className={secHeader}>Terms &amp; Conditions</div>
                    <div className="whitespace-pre-line px-4 py-2.5 text-[12px] leading-5 text-slate-700">{invoice.terms}</div>
                  </div>
                )}
              </div>
            )}
