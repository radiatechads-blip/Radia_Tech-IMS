...
export default function InvoicePreview({ invoice, taxType: taxTypeProp, onTaxTypeChange }: Props) {
  const isInteractive = onTaxTypeChange !== undefined;
  const [internalTaxType, setInternalTaxType] = useState<TaxType>(
    (invoice.taxType as TaxType) || "cgst-sgst",
  );
  ...
  const rows = useMemo(() =>
    items.map((item) => {
      ...
      const rowAmount = taxableAmount + gstAmount;
      return { ...item, taxablePerUnit, taxableAmount, discountAmount, gstAmount, finalRatePerUnit, rowAmount, rate, qty, discountPercent, taxPercent };
    }),
    [items],
  );
  ...
  const totals = useMemo(() => {
    ...
    const grandTotalBeforeRoundOff = taxable + tax;
    const grandTotal = grandTotalBeforeRoundOff + roundOffVal;

    // Prefer saved invoice summary values when present
  ...
  const handleTaxChange = (v: TaxType) => onTaxTypeChange ? onTaxTypeChange(v) : setInternalTaxType(v);
  ...
  const docLabel = invoice.billType || "Tax Invoice";
    ...
    <section className="invoice-preview-shell rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
      <div className="mx-auto w-full max-w-[900px] overflow-hidden border border-black bg-white text-slate-800 shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
        ...
        <div className="flex items-start justify-between gap-4 border-b border-slate-300 bg-white px-5 py-3">
          ...
          <div className="text-right text-[13px] dark-black-600 leading-5 shrink-0">
            <div>Phone: +91 81788 50959</div>
            <div>Email: sales@radiatech.in</div>
            <div>GSTIN: 09DDZPK0004H1ZF</div>
            <div>State: 09-Uttar Pradesh</div>
        ...
        <div className="grid grid-cols-2 border-b border-slate-300">
          <div className="border-r border-slate-300">
            <div className={secHeader}>Bill To:</div>
          ...
          <div>
            ...
            <div className="p-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-[13px] text-slate-800">
              ...
              <span className="font-semibold text-slate-900">PO No:</span><span>{invoice.poNo || "—"}</span>
              <span className="font-semibold text-slate-900">Reference No:</span><span>{invoice.placeOfSupply || "—"}</span>
            </div>
          </div>
        </div>

        {/* ── Ship To | Transportation ── */}
        <div className="grid grid-cols-2 border-b border-slate-300">
          <div className="border-r border-slate-300">
            <div className={secHeader}>Ship To:</div>
            <div className="p-3 text-[12px] leading-5 text-slate-800 whitespace-pre-line">
              {invoice.shipToAddress || "—"}
            </div>
          </div>
          <div>
            ...
            <tbody>
              {rows.map((item, i) => (
                <tr key={item.id || i} className="odd:bg-white even:bg-slate-50/50">
                  ...
                  <td className="border border-slate-300 px-1.5 py-1.5 text-right align-top">{fmt(item.taxablePerUnit)}</td>
